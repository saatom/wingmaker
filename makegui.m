...
function makegui(wing)
    ...
    h.infogroup = uibuttongroup (h.panel1, "position", [0 .08 1 .15]); %info group
    ...
    h.slider_text = uicontrol ("style", "text",
		...
		"string", "Alpha: ");
    h.lift_text = uicontrol ("style", "text",
		"parent", h.infogroup, 
		"position", [0 .66 1 .33],
		"horizontalalignment", "left",
		"backgroundcolor", c.panel,
		"string", "Lift: 0 N");
    h.drag_text = uicontrol ("style", "text",
		"parent", h.infogroup, 
		"position", [0 .33 1 .33],
		"horizontalalignment", "left",
		"backgroundcolor", c.panel,
		"string", "Drag: 0 N");
    h.moment_text = uicontrol ("style", "text",
		"parent", h.infogroup, 
		"position", [0 0 1 .33],
		"horizontalalignment", "left",
		"backgroundcolor", c.panel,
...
function updatePlot(obj, init = false)
    ...
    switch obj
	case h.alpha_slider
	    alpha=alphaFilter(get(h.alpha_slider, "value"));
	    set(h.slider_text_entry, "string", num2str(alpha));
	    replot = true;
	case h.slider_text_entry
	    alpha=alphaFilter(str2num(get(h.slider_text_entry, "string")));
	    set(h.slider_text_entry, "string", num2str(alpha));
	...
	case h.performance_button
	    ...
	    %h.wing.alphas = [-5 5 1];
	    h.wing.performance_data = performance(wing, 3e6, wing.alphas);
	...
	if size(wing.performance_data.polar) > 0
	    ...
	    data=pol(lin, 2:5);
	    set(h.lift_text, "string", ["Lift: " num2str(data(1)) " N"]);
	    set(h.drag_text, "string", ["Drag: " num2str(data(2)+data(3)) " N"]);
	    set(h.moment_text, "string", ["Moment: " num2str(data(4)) " Nm"]);
    	endif
